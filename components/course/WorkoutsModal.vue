<template>
  <Teleport to="body">
    <div
      ...
      @click.self="close"
    >
      <div class="w-full max-w-md bg-white rounded-2xl shadow-xl p-6 relative max-h-[80vh] flex flex-col">
        <!-- Кнопка закрытия -->
        <button
          @click="close"
        ...
        <h2 class="text-xl font-bold text-gray-900 mb-6">Выберите тренировку</h2>

        <!-- Загрузка -->
        <div v-if="loading" class="text-gray-500 text-center py-8">Загрузка тренировок...</div>
        ...
        <ul v-else class="space-y-2 overflow-y-auto flex-1 mb-6">
          <li
            ...
            :class="[
              selectedWorkout?._id === workout._id ? 'bg-gray-100' : 'hover:bg-gray-50'
            ]"
          ...
          >
            ...
            <div
              ...
              :class="isWorkoutCompleted(workout._id) ? 'bg-primary border-primary' : 'border-gray-300'"
            >
              <svg
            ...
            <!-- Информация -->
            <div class="flex-1">
              <p class="text-gray-900 font-medium">{{ getWorkoutTitle(workout.name) }}</p>
              <p class="text-gray-500 text-xs mt-1">{{ getWorkoutSubtitle(workout.name) }}</p>
            </div>
          </li>
        </ul>
...
const progressData = ref<Record<string, any>>({})
...
const loadWorkouts = async () => {
  ...
  try {
    ...
    error.value = err.message || 'Не удалось загрузить тренировки'
  } finally {
    loading.value = false
  }
}
...
const handleStart = () => {
  ...
  navigateTo(`/training/${selectedWorkout.value._id}?courseId=${props.courseId}`)
  close()
}
